...
The central caching abstraction. Register a concrete implementation (backed by `IDistributedCache` or `Microsoft.Extensions.Caching.Hybrid`) before calling `AddSharedKernelInfrastructure`.
...
`ICacheService` itself is registered by `AddSharedKernelInfrastructure` as `Transient<CacheService>`. **You must register a backing `IDistributedCache` before calling `AddSharedKernelInfrastructure`**, otherwise `CacheService` will have nothing to delegate to:

```csharp
// Register a distributed cache implementation first
builder.Services.AddStackExchangeRedisCache(options =>
{
    options.Configuration = builder.Configuration["Redis:ConnectionString"];
});

// Then register SharedKernel (CacheService depends on IDistributedCache)
...
<Steps>
  ...
  <Step title="Register services">
    ```csharp
    // Distributed cache (required by CacheService)
    builder.Services.AddStackExchangeRedisCache(options =>
        options.Configuration = builder.Configuration["Redis:ConnectionString"]);

    // SharedKernel infrastructure (registers CacheService)
    builder.Services.AddSharedKernelInfrastructure();
