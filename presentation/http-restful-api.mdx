---
title: "REST API Presentation Helpers in SharedKernel"
sidebarTitle: "HTTP REST API"
description: "Build ASP.NET Core HTTP APIs faster with BaseApiCommandController, BaseApiQueryController, ExceptionMiddleware, ResponseBuilder, and user/partition context middlewares."
---
...
Reads the `UserId` header from the incoming HTTP request and populates `IUserContextService` for use anywhere downstream (application layer, domain services, etc.).
...
<Note>
  Anonymous access to `/swagger`, `/health`, and `/metrics` paths is automatically allowed by `AnonymousRequestHelper.IsAnonymousAllowed`. For all other paths, the header is required or the middleware returns `400 Bad Request`.
</Note>
...
<Tip>
  Override this logic in your own middleware pipeline if you need different anonymous-access rules — for example, to allow unauthenticated access to an `api/v1/auth/login` endpoint.
</Tip>
...
The helper selects the response shape based on `statusCode`:

| Status Code | Response Type             | RFC Reference                                  |
|-------------|--------------------------|------------------------------------------------|
| `400`       | `ValidationProblemDetails` | RFC 7231 §6.5.1                              |
