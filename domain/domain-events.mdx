...
<Note>
Domain events are **collected on the aggregate** in memory and are **not dispatched until after the Unit of Work commits**. `EventsDispatcherService.DispatchEventsFromAggregatesStoreAsync` should be called just before or just after `SaveChanges`, depending on your transactional guarantees. This ensures that handlers only fire when the state change is durable.
</Note>
...
Handlers implement `IBaseEventHandler<TEvent>` from the Application contracts package:
...
```csharp
public sealed class OrderCreatedEventHandler : IBaseEventHandler<OrderCreatedEvent>
{
...
<Steps>
  ...
  <Step title="Unit of Work dispatches events">
    Before or after `SaveChanges`, your infrastructure calls `EventsDispatcherService.DispatchEventsFromAggregatesStoreAsync`. Unpublished events are published via `IEventBus` in chronological order.
  </Step>
